...
def test_multiple_strategy_withdraw_flow(
    ...
    currentIdle = vault_balance // 4
    currentDebt = vault_balance * 3 // 4

    assert vault.totalIdle() == currentIdle
    assert vault.totalDebt() == currentDebt
    assert asset.balanceOf(vault) == currentIdle
    ...
    vault.withdraw(currentIdle, whale.address, whale.address, sender=whale)

    assert asset.balanceOf(whale) == currentIdle
