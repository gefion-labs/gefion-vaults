...
def test_profitable_strategy_flow(
    ...
    set_fees_for_strategy(gov, strategy, accountant, 0, performanceFee)
    ...
    total_fee = first_profit * (performanceFee / MAX_BPS_ACCOUNTANT)
    ...
    assert vault.totalIdle() == 0
    # Les set a `minimumTotalIdle` value
    min_total_idle = deposit_amount // 2
    vault.setMinimumTotalIdle(min_total_idle, sender=gov)

    # We update debt for minimumTotalIdle to take effect
    newDebt = strategy.totalAssets() - deposit_amount // 4
    ...
    assert vault.strategies(strategy).currentDebt != newDebt

    user_1_withdraw = vault.totalIdle()
    print(
        f"Unrealized losses 1= {vault.assessShareOfUnrealisedLosses(strategy, user_1_withdraw)}"
    )
