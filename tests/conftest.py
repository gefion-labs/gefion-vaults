...
@pytest.fixture(scope="session")
def vaultOriginal(project, gov):
    vault = gov.deploy(project.Vault)
    return vault.address


@pytest.fixture(scope="session")
def vault_factory(project, gov, vaultOriginal):
    return gov.deploy(
...
def set_factory_fee_config(project, gov, vault_factory):
    def set_factory_fee_config(feeBps, feeRecipient):
        vault_factory.setProtocolFeeRecipient(feeRecipient, sender=gov)
        vault_factory.setProtocolFeeBps(feeBps, sender=gov)
...
def create_vault(project, gov, vault_factory):
    def create_vault(
        asset,
        governance=gov,
        depositLimit=MAX_INT,
        max_profit_locking_time=WEEK,
        vault_name=None,
        vault_symbol="vGEFION",
    ):
        if not vault_name:
            ...
            vault_name = f"Vault {vault_suffix}"
        ...
        vault = project.Vault.at(event[0].vaultAddress)
...
def deploy_accountant(project, gov):
    def deploy_accountant(vault):
        ...
        # set up fee manager
        vault.setAccountant(accountant.address, sender=gov)
        return accountant
...
def deploy_flexible_accountant(project, gov):
    def deploy_flexible_accountant(vault):
        ...
        # set up fee manager
        vault.setAccountant(flexible_accountant.address, sender=gov)
...
def deploy_faulty_accountant(project, gov):
    def deploy_faulty_accountant(vault):
        ...
        # set up fee manager
        vault.setAccountant(faulty_accountant.address, sender=gov)
...
def deploy_limit_module(project, gov):
    def deploy_limit_module(
        depositLimit=MAX_INT, withdraw_limit=MAX_INT, whitelist=False
    ):
        limit_module = gov.deploy(
            project.LimitModule, depositLimit, withdraw_limit, whitelist
        )
        return limit_module
...
def add_debt_to_strategy():
    def add_debt_to_strategy(user, strategy, vault, targetDebt: int):
        vault.updateMaxDebtForStrategy(strategy.address, targetDebt, sender=user)
        vault.updateDebt(strategy.address, targetDebt, sender=user)
...
def set_fees_for_strategy():
    def set_fees_for_strategy(
        gov, strategy, accountant, managementFee, performanceFee, refundRatio=0
    ):
        accountant.setManagementFee(strategy.address, managementFee, sender=gov)
        accountant.setPerformanceFee(strategy.address, performanceFee, sender=gov)
        accountant.setRefundRatio(strategy.address, refundRatio, sender=gov)
...
def initial_set_up(
    ...
    def initial_set_up(
        ...
        accountant = None
        if managementFee or performanceFee or refundRatio:
