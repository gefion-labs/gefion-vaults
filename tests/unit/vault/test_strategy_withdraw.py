...
def test_withdraw__with_multiple_liquid_strategies__withdraws(
    ...
    assert event[0].currentDebt == amount_per_strategy
    assert event[0].newDebt == 0
    assert event[1].strategy == second_strategy.address
    assert event[1].currentDebt == amount_per_strategy
    assert event[1].newDebt == 0
...
def test_withdraw__with_locked_and_liquid_strategy__withdraws(
    ...
    assert event[0].currentDebt == amount_per_strategy
    assert event[0].newDebt == amount_per_strategy - amount_to_lock
...
def test_withdraw__from_lossy_strategy_with_unrealised_losses_and_max_redeem(
    ...
    assert event[0].strategy == lossy_strategy.address
    assert event[0].currentDebt == amount_per_strategy
    assert event[0].newDebt == amount_to_lock + (amount_to_lose // 2)
...
def test_withdraw__half_of_strategy_assets_from_lossy_strategy_with_unrealised_losses__withdraws_less_than_deposited(
    ...
    assert event[0].strategy == lossy_strategy.address
    assert event[0].currentDebt == amount_per_strategy
    assert event[0].newDebt == amount_per_strategy - amount_to_withdraw
...
def test_redeem__half_of_strategy_assets_from_locked_lossy_strategy_with_unrealised_losses__withdraws_less_than_deposited(
    ...
    assert event[1].strategy == liquid_strategy.address
    assert event[1].currentDebt == amount_per_strategy
    assert event[1].newDebt == amount_per_strategy - expected_liquid_out
...
def test_withdraw__with_custom_queue_and_useDefaultQueue__overrides(
    gov,
    fish,
    fish_amount,
    asset,
    ...
    vault.setDefaultQueue([second_strategy, first_strategy], sender=gov)
    ...
    assert vault.strategies(first_strategy)["currentDebt"] == amount_per_strategy
    assert vault.strategies(second_strategy)["currentDebt"] == 0
    assert asset.balanceOf(vault) == 0
...
def test_redeem__with_custom_queue_and_useDefaultQueue__overrides(
    gov,
    fish,
    fish_amount,
    asset,
