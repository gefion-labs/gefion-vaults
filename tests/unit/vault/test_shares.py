...
def test_deposit__with_depositLimit_within_depositLimit__deposit_balance(
    fish, fish_amount, asset, create_vault, user_deposit
):
    vault = create_vault(asset, depositLimit=fish_amount)
    amount = fish_amount
    shares = amount
...
def test_deposit__with_depositLimit_exceed_depositLimit__reverts(
    fish, fish_amount, asset, create_vault
):
    amount = fish_amount
    depositLimit = amount - 1
    vault = create_vault(asset, depositLimit=depositLimit)

    with ape.reverts("exceed deposit limit"):
        vault.deposit(amount, fish.address, sender=fish)


def test_deposit_all__with_depositLimit_exceed_depositLimit__deposit_depositLimit(
    fish, fish_amount, asset, create_vault
):
    amount = fish_amount
    depositLimit = amount // 2
    vault = create_vault(asset, depositLimit=depositLimit)
...
def test_mint__with_depositLimit_within_depositLimit__deposit_balance(
    fish, fish_amount, asset, create_vault
):
    vault = create_vault(asset)
    amount = fish_amount
...
def test_mint__with_depositLimit_exceed_depositLimit__reverts(
    fish, fish_amount, asset, create_vault
):
    amount = fish_amount
    shares = amount
...
@pytest.mark.parametrize("depositLimit", [0, 10**18, MAX_INT])
def test_set_depositLimit__with_depositLimit(
    project, create_vault, gov, asset, depositLimit
):
    vault = create_vault(
        asset=asset,
        governance=gov,
    ...
    tx = vault.setDepositLimit(depositLimit, sender=gov)
...
def test__mint_shares_with_zero_total_supply_positive_assets(
    ...
    vault.updateDebt(strategy, int(0), sender=gov)
    assert (
