...
def test_buy_debt__strategy_not_active__reverts(
    ...
    with ape.reverts("not active"):
        vault.buyDebt(strategy, amount, sender=gov)
...
def test_buy_debt__no_amount__reverts(
    ...
    with ape.reverts("nothing to buy with"):
        vault.buyDebt(strategy, 0, sender=gov)


def test_buy_debt__more_than_available__withdraws_currentDebt(
    gov,
    asset,
    vault,
    mint_and_deposit_into_vault,
    ...
    tx = vault.buyDebt(strategy, amount * 2, sender=gov)
    ...
    assert logs[0].currentDebt == amount
    assert logs[0].newDebt == 0
...
def test_buy_debt__full_debt(
    ...
    tx = vault.buyDebt(strategy, amount, sender=gov)
...
def test_buy_debt__half_debt(
    ...
    tx = vault.buyDebt(strategy, to_buy, sender=gov)
    ...
    assert logs[0].strategy == strategy.address
    assert logs[0].currentDebt == amount
    assert logs[0].newDebt == amount - to_buy

    assert vault.totalIdle() == to_buy
    assert vault.totalDebt() == amount - to_buy
    assert vault.pricePerShare() == 10 ** asset.decimals()
    assert vault.strategies(strategy)["currentDebt"] == amount - to_buy
    # assert shares got moved
