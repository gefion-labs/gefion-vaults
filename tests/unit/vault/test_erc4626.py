...
@pytest.mark.parametrize("depositLimit", ["half_fish_amount", "fish_amount"])
def test_max_deposit__with_total_assets_greater_than_or_equal_depositLimit__returns_zero(
    asset, fish, fish_amount, gov, create_vault, depositLimit, user_deposit, request
):
    vault = create_vault(asset)
    assets = fish_amount
    ...
    depositLimit = request.getfixturevalue(depositLimit)

    vault.setDepositLimit(depositLimit, sender=gov)

    assert vault.maxDeposit(fish.address) == 0


def test_max_deposit__with_total_assets_less_than_depositLimit__returns_net_depositLimit(
    asset, fish, fish_amount, gov, create_vault
):
    vault = create_vault(asset)
    assets = fish_amount
    depositLimit = assets // 2

    vault.setDepositLimit(depositLimit, sender=gov)

    assert vault.maxDeposit(fish.address) == depositLimit
...
@pytest.mark.parametrize("depositLimit", ["half_fish_amount", "fish_amount"])
def test_max_mint__with_total_assets_greater_than_or_equal_depositLimit__returns_zero(
    asset, fish, fish_amount, gov, create_vault, depositLimit, user_deposit, request
):
    vault = create_vault(asset)
    assets = fish_amount
...
def test_max_mint__with_total_assets_less_than_depositLimit__returns_net_depositLimit(
    asset, fish, fish_amount, gov, create_vault
):
    vault = create_vault(asset)
    assets = fish_amount
    ...
    assert vault.maxMint(fish.address) == depositLimit
...
def test_max_withdraw__with_balance_greater_than_total_idle__returns_balance(
    ...
    totalIdle = assets - strategy_deposit
...
def test_max_withdraw__with_lossy_strategy(
    ...
    # Should return just idle if maxLoss == 0.
    assert vault.maxWithdraw(fish.address) == totalIdle
...
def test_max_withdraw__with_liquid_and_lossy_strategy(
    ...
    # With a default maxLoss of 0 Should return just the first strategies debt
    ...
    # With a 0 maxLoss but the lossy first it should return 0.
...
def test_max_withdraw__with_useDefaultQueue(
    asset,
    fish,
    fish_amount,
    gov,
    ...
    # Set useDefaultQueue to true
    vault.setUseDefaultQueue(True, sender=gov)
...
def test_max_redeem__with_lossy_strategy(
    ...
    # Should return just idle if maxLoss == 0.
    assert vault.maxRedeem(fish.address, 0) == totalIdle
...
def test_max_redeem__with_liquid_and_lossy_strategy(
    ...
    # With a maxLoss of 0 Should return just the first strategies debt
...
def test_max_redeem__with_useDefaultQueue(
    asset,
    fish,
    fish_amount,
    gov,
...
def test_max_deposit__with_depositLimit_module(
    asset, fish, fish_amount, gov, create_vault, deploy_limit_module, user_deposit
):
    vault = create_vault(asset, depositLimit=0)
    ...
    assert vault.depositLimitModule() == limit_module.address
    assert vault.maxDeposit(fish.address) == MAX_INT

    new_limit = assets * 2
    limit_module.setDefaultDepositLimit(new_limit, sender=gov)
    ...
    limit_module.setEnforceWhitelist(True, sender=gov)

    assert vault.maxDeposit(fish.address) == 0

    # If whitelisted it now works
    limit_module.setWhitelist(fish.address, sender=gov)

    assert vault.maxDeposit(fish.address) == new_limit - assets


def test_max_mint__with_depositLimit_module(
    asset, fish, fish_amount, gov, create_vault, deploy_limit_module, user_deposit
):
    vault = create_vault(asset, depositLimit=0)
    limit_module = deploy_limit_module()
...
def test_max_withdraw__with_withdraw_limit_module(
    ...
    vault.setWithdrawLimitModule(limit_module, sender=gov)

    assert vault.withdrawLimitModule() == limit_module.address
    ...
    assert limit_module.defaultWithdrawLimit() == MAX_INT
    assert vault.maxWithdraw(fish.address) == assets
    assert vault.maxWithdraw(bunny.address) == 0

    new_limit = assets * 2
    limit_module.setDefaultWithdrawLimit(new_limit, sender=gov)
...
def test_deposit__with_depositLimit_module(
    asset, fish, fish_amount, gov, create_vault, deploy_limit_module, user_deposit
):
    vault = create_vault(asset, depositLimit=0)
    limit_module = deploy_limit_module()
...
def test_mint__with_depositLimit_module(
    asset, fish, fish_amount, gov, create_vault, deploy_limit_module, user_deposit
):
    vault = create_vault(asset, depositLimit=0)
    limit_module = deploy_limit_module()
